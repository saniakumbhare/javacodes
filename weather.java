...
public class weather {
    ...
    public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, 
            ...
            for (int i = 2; i < drywetProbability.length + 2;i++) {
                drywetProbability[a] = drywet[x][i];
                a++;
            }
            ...
            for (int i = 2; i < wetwetProbability.length + 2; i++) {
                wetwetProbability[a] = wetwet[x][i];
                a++;
            }
    ...
    public static int[] oneMonthForecast(int numberOfLocations, int month, double longitude, double latitude ){
        ...
        int[] oneMonth = forecastGenerator(drywetProb[month], wetwetProb[month], numberOfDaysInMonth[month]);
    ...
    public static int bestWeekToTravel(int[] forecast){
       ...
       for (int i = 0; i < forecast.length; i++) {
           while (i < forecast.length-1) {
               for (int j = 0; j < 7 && i + j < forecast.length; j++) {
                   if(forecast[i+j] == DRY) {
                       days++;
                   }
               }
